...
function buildGrid() {
  ...
  loadEmployees().then((employees) => {
    for (const key in employees) {
      ...
      row.insertCell(2).innerHTML = employee.extension;
      row.insertCell(3).innerHTML = employee.email;
      row.insertCell(4).innerHTML = employee.department;
      row.insertCell(5).innerHTML = '<button class="btn btn-danger" onclick="deleteRow(this)">Delete</button>';
